...
double calc_score(struct MLNode * node){
    ...
    double ES_calc = SAFE_DIVISION(node->references, node->ES);

    result = (error_calc + ES_calc);
...
struct MLNode * get_victim(){
    ...
    && (cold_clean->size > (BUFFER_SIZE * read_miss_intensity) ) ){
        nodes[0] = P_CC;
        nodes[1] = NULL;
        nodes[2] = NULL;
        nodes[3] = NULL;
    ...
    && (cold_dirty->size > (BUFFER_SIZE * write_miss_intensity) ) ){
        nodes[0] = NULL;
        nodes[1] = P_CD;
        nodes[2] = NULL;
        nodes[3] = NULL;
