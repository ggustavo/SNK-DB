...
struct MLNode * get_victim(){
    ...
    desirable_cold_clean_size = desirable_cold_clean_size;
    desirable_cold_dirty_size = desirable_cold_dirty_size;
    ...
    && (cold_clean->size > (BUFFER_SIZE  * (read_miss_intensity ) //* read_ghost_hit_intensity
    ...
    && (cold_dirty->size > (BUFFER_SIZE * ( write_miss_intensity ) //* write_ghost_hit_intensity
    ) ) ){
        nodes[0] = NULL;
        nodes[1] = P_CD;
        nodes[2] = NULL;
